...
local AverageHubFolder = "AverageHub"
local ConfigurationFolder = AverageHubFolder.."/Configurations"
local ConfigurationExtension = ".rfld"



local AverageHubLibrary = {
	Flags = {},
	Theme = {
		Default = {
...
local AverageHub = useStudio and script.Parent:FindFirstChild('AverageHub') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned

repeat
	if AverageHub:FindFirstChild('Build') and AverageHub.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('AverageHub | Build Mismatch')
		print('AverageHub may encounter issues as you are running an incompatible interface version ('.. ((AverageHub:FindFirstChild('Build') and AverageHub.Build.Value) or 'No Build') ..').\n\nThis version of AverageHub is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, AverageHub = AverageHub, useStudio and script.Parent:FindFirstChild('AverageHub') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

AverageHub.Enabled = false

if gethui then
	AverageHub.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(AverageHub)
	AverageHub.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	AverageHub.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == AverageHub.Name and Interface ~= AverageHub then
...
if AverageHub.AbsoluteSize.X < minSize.X and AverageHub.AbsoluteSize.Y < minSize.Y then
...
local Main = AverageHub.Main
local MPrompt = AverageHub:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = AverageHub:FindFirstChild('Drag')
local dragInteract = dragBar and dragBar.Interact or nil
local dragBarCosmetic = dragBar and dragBar.Drag or nil

AverageHub.DisplayOrder = 100
...
local Notifications = AverageHub.Notifications

local SelectedTheme = AverageHubLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AverageHubLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	AverageHub.Main.BackgroundColor3 = SelectedTheme.Background
	AverageHub.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	AverageHub.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	AverageHub.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	AverageHub.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	AverageHub.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	AverageHub.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	AverageHub.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	...
	for _, text in ipairs(AverageHub:GetDescendants()) do
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, Flag in pairs(AverageHubLibrary.Flags) do
		...
		if FlagValue then
			...
			warn("AverageHub | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AverageHubLibrary:Notify({Title = "AverageHub Flags", Content = "AverageHub was unable to find '"..FlagName.. "' in the save file. Open a ticket for help. ", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AverageHubLibrary.Flags) do
...
function AverageHubLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('AverageHub | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AverageHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show AverageHub'.", Duration = 7, Image = 4400697855})
		else
			AverageHubLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K.", Duration = 7, Image = 4400697855})
		end
	end
...
function AverageHubLibrary:CreateWindow(Settings)
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AverageHubLibrary:Notify({Title = 'Build Mismatch', Content = 'AverageHub may encounter issues as you are running an incompatible interface version ('.. ((AverageHub:FindFirstChild('Build') and AverageHub.Build.Value) or 'No Build') ..').\n\nThis version of AverageHub is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AverageHub"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "AverageHub Interface Suite" then
		LoadingFrame.Version.Text = "AverageHub UI"
	end
	...
	if not Settings.DisableAverageHubPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AverageHubLibrary:Notify({
					Title = "AverageHub Interface",
					Content = "Enjoying this script? Join our discord server!",
					Duration = 4,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(AverageHubFolder.."/Discord Invites") then
			makefolder(AverageHubFolder.."/Discord Invites")
		end

		if isfile and not isfile(AverageHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(AverageHubFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AverageHub RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(AverageHubFolder.."/Key System") then
			makefolder(AverageHubFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("AverageHub | "..Key.." Error " ..tostring(Response))
					warn('Key error')
				end
			end
		end
		...
		if isfile and isfile(AverageHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(AverageHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(AverageHubFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						AverageHubLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AverageHub | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.Aki.menu for help with AverageHub specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AverageHubLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AverageHubLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			AverageHub.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("AverageHub | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.Aki.menu for help with AverageHub specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AverageHubLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("AverageHub | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.Aki.menu for help with AverageHub specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AverageHubLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AverageHub | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.Aki.menu for help with AverageHub specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AverageHubLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AverageHubLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AverageHub | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.Aki.menu for help with AverageHub specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					AverageHubLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("AverageHub | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.Aki.menu for help with AverageHub specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AverageHubLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AverageHubLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AverageHubLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
...
function AverageHubLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AverageHubLibrary:IsVisible(): boolean
	return not Hidden
end

function AverageHubLibrary:Destroy()
	AverageHub:Destroy()
end
...
function AverageHubLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AverageHubLibrary:Notify({Title = "AverageHub Configurations", Content = "We couldn't enable Configuration Saving as you are not using file supported software.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AverageHubLibrary:Notify({Title = "AverageHub Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('AverageHub Configurations Error | '..tostring(result))
			AverageHubLibrary:Notify({Title = "AverageHub Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
end
...
if useStudio then
	...
	local Window = AverageHubLibrary:CreateWindow({
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using AverageHub may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like AverageHub to get the key from
	...
	--AverageHubLibrary:Notify({Title = "AverageHub Interface", Content = "Welcome to AverageHub. These - are the brand new notification design for AverageHub, with custom sizing and AverageHub calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(AverageHubLibrary.Theme) do
...
task.delay(4, function() 
	AverageHubLibrary.LoadConfiguration()
