...
AverageHub Interface Suite

]]

local Release = "Prerelease Beta 6b"

local AverageHub = { Folder = "AverageHub", Options = {}, ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(117, 164, 206)), ColorSequenceKeypoint.new(0.50, Color3.fromRGB(123, 201, 201)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(224, 138, 175))} }
...
local AverageHubUI = isStudio and script.Parent:WaitForChild("AverageHub UI") or game:GetObjects("rbxassetid://86467455075715")[1]
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		AverageHub:Notification({Title = "Interface Hidden", Content = "The interface has been hidden, you may reopen the interface by Pressing the UI Bind In Settings ("..tostring(bind)..")", Icon = "visibility_off"})
	end
...
if gethui then
	AverageHubUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(AverageHubUI)
	AverageHubUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	AverageHubUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == AverageHubUI.Name and Interface ~= AverageHubUI then
			Hide(Interface.SmartWindow)
			Interface.Enabled = false
			Interface.Name = "AverageHub-Old"
		end
	end
elseif not isStudio then
...
AverageHubUI.Enabled = false
AverageHubUI.SmartWindow.Visible = false
AverageHubUI.Notifications.Template.Visible = false
AverageHubUI.DisplayOrder = 1000000000

local Main : Frame = AverageHubUI.SmartWindow
local Dragger = Main.Drag
local dragBar = AverageHubUI.Drag
...
local Notifications = AverageHubUI.Notifications
...
-- 	for FlagName, Flag in pairs(AverageHub.Flags) do
...
-- 		else
-- 			notified = true
-- 			AverageHub:Notification({Title = "Config Error", Content = "AverageHub was unable to load or find '"..FlagName.. "'' in the current script. Check ".. website .." for help.", Icon = "flag"})
-- 		end
-- 	end
-- 	if autoload and notified == false then
-- 		AverageHub:Notification({
-- 			Title = "Config Autoloaded",
-- 			Content = "The Configuration Has Been Automatically Loaded. Thank You For Using AverageHub Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
...
-- 		AverageHub:Notification({
-- 			Title = "Config Loaded",
-- 			Content = "The Configuration Has Been Loaded. Thank You For Using AverageHub Library",
-- 			Icon = "file-code-2",
-- 			ImageSource = "Lucide"
-- 		})
-- 	end
...
-- 	local Data = {}
-- 	for i,v in pairs(AverageHub.Flags) do
...
function AverageHub:Notification(data) -- action e.g open messages
	task.spawn(function()
		data = Kwargify({
			Title = "Missing Title",
...
function AverageHub:CreateWindow(WindowSettings)

	WindowSettings = Kwargify({
		Name = "AverageHub UI Example Window",
		Subtitle = "",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "AverageHub Interface Suite",
	...
	LoadingFrame.Version.Text = LoadingFrame.Frame.Frame.Title.Text == "AverageHub Interface Suite" and Release or "AverageHub UI"
	...
	AverageHubUI.Enabled = true

	BlurModule(Main)

	if WindowSettings.KeySystem then
		Draggable(Dragger, Main)
		Draggable(AverageHubUI.MobileSupport, AverageHubUI.MobileSupport)
		...
		local direc = WindowSettings.KeySettings.SaveInRoot and "AverageHub/Configurations/" .. WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/" or "AverageHub/Configurations/" ..  WindowSettings.ConfigSettings.ConfigFolder .. "/Key System/"

		if isfile and isfile(direc .. WindowSettings.KeySettings.FileName .. ".AverageHub") then
			for i, Key in ipairs(WindowSettings.KeySettings.Key) do
				if string.find(readfile(direc .. WindowSettings.KeySettings.FileName .. ".AverageHub"), Key) then
					Passthrough = true
					break
				end
			end
			...
			KeySystem.Action.Interact.MouseButton1Click:Connect(function()
				...
				if KeyFound then 
					...
					if WindowSettings.KeySettings.SaveKey then
						if writefile then
							writefile(direc .. WindowSettings.KeySettings.FileName .. ".AverageHub", FoundKey)
						end
						AverageHub:Notification({Title = "Key System", Content = "The key for this script has been saved successfully.", Icon = "lock_open"})
					end
				else
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "AverageHub Interface Suite - Debugging Mode" break end
	...
	function Window:CreateTab(TabSettings)
		...
		function Tab:CreateSection(name : string)
			...
			function Section:CreateButton(ButtonSettings)
				...
				Button.Interact["MouseButton1Click"]:Connect(function()
					...
					if not Success then
						...
						print("AverageHub Interface Suite | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Button.Title.Text = ButtonSettings.Name
						TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
			...
			function Section:CreateSlider(SliderSettings, Flag)
						...
						if SLDragging then
							...
							if SliderSettings.CurrentValue ~= NewValue then
								...
								if not Success then
									...
									print("AverageHub Interface Suite | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
									wait(0.5)
									Slider.Title.Text = SliderSettings.Name
									TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
								...
								-- AverageHub.Flags[SliderSettings.Flag] = SliderSettings
							end
						else
				...
				if Flag then
					AverageHub.Options[Flag] = SliderV
				end

				AverageHubUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Slider.Main.color.Color = AverageHub.ThemeGradient
					Slider.Main.UIStroke.color.Color = AverageHub.ThemeGradient
				end)
			...
			function Section:CreateToggle(ToggleSettings, Flag)    
				...
				Toggle.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("AverageHub Interface Suite | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Toggle.Title.Text = ToggleSettings.Name
						TweenService:Create(Toggle, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.5}):Play()
				...
				AverageHubUI.ThemeRemote:GetPropertyChangedSignal("Value"):Connect(function()
					Toggle.toggle.color.Color = AverageHub.ThemeGradient
					Toggle.toggle.UIStroke.color.Color = AverageHub.ThemeGradient
				end)

				if Flag then
					AverageHub.Options[Flag] = ToggleV
				end
			...
			function Section:CreateBind(BindSettings, Flag)
					...
					if CheckingForKey then
						if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Window.Bind then
							...
							if not Success then
								...
								print("AverageHub Interface Suite | "..BindSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Bind.Title.Text = BindSettings.Name
				...
				if Flag then
					AverageHub.Options[Flag] = BindV
				end

				-- AverageHub.Flags[BindSettings.Flag] = BindSettings
			...
			function Section:CreateInput(InputSettings, Flag)
					...
					if InputSettings.Enter then
						if bleh then
							...
							if not Success then
								...
								print("AverageHub Interface Suite | "..InputSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Input.Title.Text = InputSettings.Name
				...
				if Flag then
					AverageHub.Options[Flag] = InputV
				end
			...
			function Section:CreateDropdown(DropdownSettings, Flag)
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("AverageHub Interface Suite | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
				...
				function DropdownV:Set(NewDropdownSettings)
					...
					-- AverageHub.Flags[DropdownSettings.Flag] = DropdownSettings
				...
				if Flag then
					AverageHub.Options[Flag] = DropdownV
				end
			...
			function Section:CreateColorPicker(ColorPickerSettings, Flag) -- by Rayfield/Throit
				...
				local function SafeCallback(param, c2)
					...
					if not Success then
						...
						print("AverageHub Interface Suite | "..ColorPickerSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						ColorPicker.Title.Text = ColorPickerSettings.Name
				...
				if Flag then
					AverageHub.Options[Flag] = ColorPickerV
				end
		...
		function Tab:BuildConfigSection()
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					local success, returned = AverageHub:SaveConfig(inputPath)
					if not success then
						AverageHub:Notification({
							Title = "Interface",
							Icon = "error",
					...
					configSelection:Set({ Options = AverageHub:RefreshConfigList() })
				end
			})
			...
			configSelection = Tab:CreateDropdown({
				...
				Options = AverageHub:RefreshConfigList(),
				CurrentOption = {},
				MultipleOptions = false,
				SpecialType = nil,
				Callback = function(Value)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = AverageHub:LoadConfig(selectedConfig)
					if not success then
						AverageHub:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					local success, returned = AverageHub:SaveConfig(selectedConfig)
					if not success then
						AverageHub:Notification({
							Title = "Interface",
							Icon = "error",
			...
			Tab:CreateButton({
				...
				Callback = function()
					...
					writefile(AverageHub.Folder .. "/settings/autoload.txt", name)
			...
			Tab:CreateButton({
				...
				Callback = function()
					local name = selectedConfig
					delfile(AverageHub.Folder .. "/settings/autoload.txt")
			...
			if isfile(AverageHub.Folder .. "/settings/autoload.txt") then
				local name = readfile(AverageHub.Folder .. "/settings/autoload.txt")
				loadlabel:Set( { Text = "Current autoload config: " .. name })
			end     
		end
		...
		local ClassParser = {
			["Toggle"] = {
				...
				Load = function(Flag, data)
					if AverageHub.Options[Flag] then
						AverageHub.Options[Flag]:Set({ CurrentValue = data.state })
					end
				end
			},
			["Slider"] = {
				...
				Load = function(Flag, data)
					if AverageHub.Options[Flag] and data.value then
						AverageHub.Options[Flag]:Set({ CurrentValue = data.value })
					end
				end
			},
			["Input"] = {
				...
				Load = function(Flag, data)
					if AverageHub.Options[Flag] and data.text and type(data.text) == "string" then
						AverageHub.Options[Flag]:Set({ CurrentValue = data.text })
					end
				end
			},
			["Dropdown"] = {
				...
				Load = function(Flag, data)
					if AverageHub.Options[Flag] and data.value then
						AverageHub.Options[Flag]:Set({ CurrentOption = data.value })
					end
				end
			},
			-- buggy as hell stil
			["Colorpicker"] = {
				...
				Load = function(Flag, data)
					...
					if AverageHub.Options[Flag] and data.color then
						AverageHub.Options[Flag]:Set({Color = HexToColor3(data.color)})
					end
				end
			}
		}
		...
		function Tab:BuildThemeSection()
			...
			}, "AverageHubInterfaceSuitePrebuiltCPC1") -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps

			local c2cp = Tab:CreateColorPicker({
				Name = "Color 2",
				Color = Color3.fromRGB(123, 201, 201),
			}, "AverageHubInterfaceSuitePrebuiltCPC2")

			local c3cp = Tab:CreateColorPicker({
				Name = "Color 3",
				Color = Color3.fromRGB(224, 138, 184),
			}, "AverageHubInterfaceSuitePrebuiltCPC3") 
			...
			c1cp:Set({
				Callback = function(Value)
					if c2cp and c3cp then
						AverageHub.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						AverageHubUI.ThemeRemote.Value = not AverageHubUI.ThemeRemote.Value
					end
				end
			})
			...
			c2cp:Set({
				Callback = function(Value)
					if c1cp and c3cp then
						AverageHub.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, Value or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, c3cp.Color or Color3.fromRGB(255,255,255))}
						AverageHubUI.ThemeRemote.Value = not AverageHubUI.ThemeRemote.Value
					end
				end
			})
			...
			c3cp:Set({
				Callback = function(Valuex)
					if c2cp and c1cp then
						AverageHub.ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, c1cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(0.50, c2cp.Color or Color3.fromRGB(255,255,255)), ColorSequenceKeypoint.new(1.00, Valuex or Color3.fromRGB(255,255,255))}
						AverageHubUI.ThemeRemote.Value = not AverageHubUI.ThemeRemote.Value
					end
				end
			})
		...
		local function SetFolder()
			...
			if WindowSettings.ConfigSettings.RootFolder ~= nil and WindowSettings.ConfigSettings.RootFolder ~= "" then
				AverageHub.Folder = WindowSettings.ConfigSettings.RootFolder .. "/" .. WindowSettings.ConfigSettings.ConfigFolder
			else
				AverageHub.Folder = WindowSettings.ConfigSettings.ConfigFolder
			end
		...
		function AverageHub:SaveConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local fullPath = AverageHub.Folder .. "/settings/" .. Path .. ".AverageHub"

			local data = {
				objects = {}
			}

			for flag, option in next, AverageHub.Options do
		...
		function AverageHub:LoadConfig(Path)
			if isStudio then return "Config system unavailable." end
			...
			local file = AverageHub.Folder .. "/settings/" .. Path .. ".AverageHub"
		...
		function AverageHub:LoadAutoloadConfig()
			if isfile(AverageHub.Folder .. "/settings/autoload.txt") then
				...
				local success, err = AverageHub:LoadConfig(name)
				if not success then
					return AverageHub:Notification({
						Title = "Interface",
						Icon = "sparkle",
						ImageSource = "Material",
		...
		function AverageHub:RefreshConfigList()
			if isStudio then return "Config system unavailable." end

			local list = listfiles(AverageHub.Folder .. "/settings")

			local out = {}
			for i = 1, #list do
				local file = list[i]
				if file:sub(-5) == ".AverageHub" then
					local pos = file:find(".AverageHub", 1, true)
					local start = pos
	...
	Main.Controls.Close.ImageLabel.MouseButton1Click:Connect(function()
		...
		if UserInputService.KeyboardEnabled == false then
			AverageHubUI.MobileSupport.Visible = true
		end
	end)
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == Window.Bind then
			...
			AverageHubUI.MobileSupport.Visible = false
	...
	AverageHubUI.MobileSupport.Interact.MouseButton1Click:Connect(function()
		Unhide(Main, Window.CurrentTab)
		dragBar.Visible = true
		Window.State = true
		AverageHubUI.MobileSupport.Visible = false
...
function AverageHub:Destroy()
	...
	AverageHubUI:Destroy()
end

if isStudio then
	local Window = AverageHub:CreateWindow({
		Name = "Nebula Client - AverageHub Hub | Blade Ball",
		Subtitle = "by Akira",
		LogoID = "123795201100198",
		LoadingEnabled = true,
		LoadingTitle = "Nebula Client (AverageHub Hub)",
	...
	--[[local Window = AverageHub:CreateWindow({
		Name = "AverageHub Example Window",
		Subtitle = "Test",
		LogoID = "6031097225",
		LoadingEnabled = true,
		LoadingTitle = "AverageHub Interface Suite",
	...
	AverageHub:Notification({ 
		Title = "Welcome to AverageHub",
		Icon = "sparkle",
		ImageSource = "Material",
		Content = "Welcome to the AverageHub Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Akira. AverageHub was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. AverageHub Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using AverageHub Today at " .. website
	})
	...
	Tabs.Main2:CreateDropdown({
		...
		Description = "AverageHub's Dropdowns Has a built in Player Dropdown!",
...
--[[if isStudio then
    window = AverageHub:CreateWindow({LoadingEnabled = false})
    t1 = window:CreateTab()
    ...
    t1:CreateParagraph({Text = "Welcome to the AverageHub Interface Suite. This Is an Amazing Quality Freemium UI Library For Roblox Exploiting Made By Akira. AverageHub was Created in hopes of improving the standard of UI Library designs by being the golden standard for it. AverageHub Has Amazing Features like a key system, notification and perfection in aesthetics and design. So, What Are You Waiting For? Start Using AverageHub Today at " .. website})
